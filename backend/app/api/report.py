...
logger = get_logger('microfish.api.report')
...
def generate_report():
    try:
        ...
        if not storage:
            return jsonify({"success": False, "error": "GraphStorage not initialized -- check graph storage connection"}), 500
...
def download_report_pdf(report_id: str):
    ...
    try:
        ...
        if os.path.exists(md_path):
            ...
            return jsonify({"success": False, "error": "No report content available"}), 404

        import tempfile
        html_content = _markdown_to_html(markdown_content, report_id)

        temp_html = tempfile.NamedTemporaryFile(mode='w', suffix='.html', delete=False, encoding='utf-8')
        temp_html.write(html_content)
        temp_html.close()

        return send_file(temp_html.name, as_attachment=True, download_name=f"report_{report_id[:8]}.html", mimetype='text/html')

    except Exception as e:
        logger.error(f"Failed to download report as PDF: {str(e)}")
        return jsonify({"success": False, "error": str(e), "traceback": traceback.format_exc()}), 500


def _markdown_to_html(markdown_content: str, report_id: str) -> str:
    """Convert markdown to a styled HTML document suitable for printing to PDF"""
    import re

    lines = markdown_content.split('\n')
    html_parts = ['<!DOCTYPE html><html><head><meta charset="utf-8">']
    html_parts.append('<title>Prediction Report</title>')
    html_parts.append('<style>')
    html_parts.append('body { font-family: "Segoe UI", Arial, sans-serif; max-width: 900px; margin: 0 auto; padding: 40px 60px; color: #1a1a2e; line-height: 1.8; background: #fff; }')
    html_parts.append('h1 { font-size: 28px; color: #0a0e1a; border-bottom: 3px solid #3b82f6; padding-bottom: 12px; margin-top: 40px; }')
    html_parts.append('h2 { font-size: 22px; color: #1e3a5f; border-bottom: 1px solid #e0e7ff; padding-bottom: 8px; margin-top: 32px; }')
    html_parts.append('h3 { font-size: 18px; color: #334155; margin-top: 24px; }')
    html_parts.append('p { margin: 12px 0; text-align: justify; }')
    html_parts.append('ul, ol { padding-left: 24px; }')
    html_parts.append('li { margin: 6px 0; }')
    html_parts.append('blockquote { border-left: 4px solid #3b82f6; margin: 16px 0; padding: 12px 20px; background: #f0f4ff; color: #334155; }')
    html_parts.append('code { background: #f1f5f9; padding: 2px 6px; border-radius: 4px; font-size: 0.9em; }')
    html_parts.append('pre { background: #0f172a; color: #e2e8f0; padding: 16px; border-radius: 8px; overflow-x: auto; }')
    html_parts.append('pre code { background: none; color: inherit; }')
    html_parts.append('table { border-collapse: collapse; width: 100%; margin: 16px 0; }')
    html_parts.append('th, td { border: 1px solid #e2e8f0; padding: 10px 14px; text-align: left; }')
    html_parts.append('th { background: #f8fafc; font-weight: 600; }')
    html_parts.append('hr { border: none; border-top: 2px solid #e2e8f0; margin: 32px 0; }')
    html_parts.append('.header-badge { display: inline-block; background: linear-gradient(135deg, #3b82f6, #06b6d4); color: white; padding: 4px 12px; border-radius: 4px; font-size: 12px; font-weight: 600; letter-spacing: 1px; margin-bottom: 8px; }')
    html_parts.append('.footer { text-align: center; color: #94a3b8; font-size: 12px; margin-top: 60px; padding-top: 20px; border-top: 1px solid #e2e8f0; }')
    html_parts.append('@media print { body { padding: 20px; } }')
    html_parts.append('</style></head><body>')
    html_parts.append('<div class="header-badge">INFER AI - PREDICTION REPORT</div>')
    ...
    for line in lines:
        ...
        if stripped.startswith('# '):
            ...
            code = re.sub(r'`(.*?)`', r'<code>\1</code>', italic)
            html_parts.append(f'<p>{code}</p>')
    ...
    html_parts.append(f'<div class="footer">Generated by Infer AI | Report ID: {report_id[:8]}</div>')
...
def chat_with_report_agent():
    try:
        ...
        storage = current_app.extensions.get('graph_storage')
        if not storage:
            raise ValueError("GraphStorage not initialized -- check graph storage connection")
        graph_tools = GraphToolsService(storage=storage)
