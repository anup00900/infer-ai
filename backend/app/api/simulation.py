...
logger = get_logger('microfish.api.simulation')
    ...
    Request (JSON):
        {
            ...
            "graph_id": "microfish_xxxx",    // Optional, if not provided, get from project
...
def prepare_simulation():
    ...
    Request (JSON):
        {
            ...
            "force_regenerate": false                     // Optional, force generate, default false
        }
    ...
    try:
        ...
        # This way frontend can immediately get expected total agents when calling prepare
        try:
            ...
            logger.warning(f"Synchronously get entity count failed (will retry in background task): {e}")
    ...
    try:
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-set total_rounds, if not then use recommended rounds
            ...
            # Get associated report_id (find this simulation's latest report)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Real-time get profile failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    try:
        ...
        logger.error(f"Real-time get config failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def generate_profiles():
    """
    Generate OASIS Agent Profile directly from knowledge graph (without creating simulation)
    
    Request (JSON):
        {
            "graph_id": "microfish_xxxx",     // Required
    ...
    try:
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": "Enable knowledge graph memory update requires valid graph_id, please ensure project graph is built"
                }), 400
...
def delete_simulation(simulation_id: str):
    ...
    try:
        ...
        if state.project_id:
            try:
                ProjectManager.delete_project(state.project_id)
            except Exception:
                pass

        logger.info(f"Simulation deleted: {simulation_id}")
        return jsonify({"success": True, "message": f"Simulation {simulation_id} deleted successfully"})
    except Exception as e:
