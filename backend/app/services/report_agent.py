...
logger = get_logger('microfish.report_agent')
...
class ReportConsoleLogger:
    ...
    def _setup_file_handler(self):
        ...
        loggers_to_attach = [
            'microfish.report_agent',
            'microfish.graph_tools',
        ]
...
- ✅ ALWAYS write the report entirely in English, regardless of input language
- ❌ Not an analysis of the current state of the real world
- ❌ Not a general overview of public sentiment
- ❌ NEVER write in Chinese or any non-English language
...
- ALWAYS write entirely in English, translating any non-English content
...
3. [Language Consistency - ALWAYS Write in English]
   - The report MUST be written entirely in English regardless of the language of source material
   - Tool returned content may contain non-English expressions -- translate everything to fluent English
   - When you quote content from tools, translate it to clear English before including it in the report
   - When translating, preserve the original meaning and ensure natural English expression
...
[Correct Example]
```
This section analyzes how public sentiment spread across platforms after the event. Through deep analysis of simulation data, we found...

**Initial Outbreak Phase**

Twitter, as the primary venue for breaking news, served as the first channel for information dissemination:

> "Twitter accounted for 68% of initial discourse volume..."

**Amplification Phase**

Reddit and other platforms further amplified the impact of the event:

- Detailed analytical threads
- High emotional resonance in comment sections
```
...
[Rules]
1. ALWAYS respond in English, regardless of the language of the question or report content
2. Prioritize answering questions based on the above report content
3. Answer questions directly, avoid lengthy deliberation
4. Only call tools to retrieve more data if the report content is insufficient to answer
5. Answers should be concise, clear, and well-organized
...
class ReportAgent:
    ...
    def _generate_section_react(
        ...
        logger.warning(f"Section {section.title} reached maximum iterations count, force generate")
        ...
        # Check force conclusion when LLM return is None
        if response is None:
    ...
    def generate_report(
        ...
        # If not provided report_id, then auto-generate
        if not report_id:
        ...
        try:
            ...
            # Phase 2: Sequentially generate sections (per section save)
            ...
            # Using ReportManager to assemble complete report
    ...
    def chat(
        ...
        In chat, Agent can autonomously call retrieval tools to answer questions
        ...
        try:
            ...
            if report and report.markdown_content:
                # Limit report length, avoid overly long context
        ...
        for iteration in range(max_iterations):
            ...
            tool_results = []
            for call in tool_calls[:1]:  # at most execute 1 tool call
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        This is the console output log (INFO, WARNING, etc.) generated during
        report generation, different from agent_log.jsonl structured logs.
        
        Args:
            report_id: Report ID
            from_line: start reading from this line (for incremental get, 0 means from the beginning)
        ...
        with open(log_path, 'r', encoding='utf-8') as f:
            for i, line in enumerate(f):
                total_lines = i + 1
                if i >= from_line:
                    # Keep original log line, remove trailing newline character
    ...
    def get_console_log_stream(cls, report_id: str) -> List[str]:
        """
        Get complete console log (one-time get all)
    ...
    def get_agent_log_stream(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get complete Agent log (for one-time get all)
    ...
    def save_section(
        ...
        Called immediately after each section generation completes, implementing per-section output
        ...
        # Build section Markdown content - clean possible duplicate title
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Check whether it is a duplicate of the section title (skip duplicates within first 5 lines)
                if i < 5:
                ...
                # because section title is added by system, content should not have any title
        ...
        # Remove beginning separator lines
        while cleaned_lines and cleaned_lines[0].strip() in ['---', '***', '___']:
            cleaned_lines.pop(0)
            # Also remove empty lines after separator line
            while cleaned_lines and cleaned_lines[0].strip() == '':
                cleaned_lines.pop(0)
    ...
    def get_generated_sections(cls, report_id: str) -> List[Dict[str, Any]]:
        """
        Get already generated section list
        
        Return all already saved section file information
        """
        folder = cls._get_report_folder(report_id)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        Assemble complete report from saved section files, and process title cleaning
        """
        folder = cls._get_report_folder(report_id)
        ...
        # Post-processing: clean entire report title issues
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep report main title (#) and section title (##), remove other level titles (###, #### etc)
        ...
        while i < len(lines):
            ...
            # Check whether this is a title line
            heading_match = re.match(r'^(#{1,6})\s+(.+)$', stripped)
            ...
            if heading_match:
                ...
                # Check whether this is a duplicate title (same content title appearing within consecutive 5 lines)
                is_duplicate = False
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Section title erroneously using #, corrected to ##
                    ...
                    if title in section_titles or title == outline.title:
                        ...
                        # Non-section second-level titles convert to bold
                        processed_lines.append(f"**{title}**")
                        processed_lines.append("")
                        prev_was_heading = False
                else:
                    # ### and below levels: titles convert to bold text
                    processed_lines.append(f"**{title}**")
                    processed_lines.append("")
                    prev_was_heading = False
                ...
                # Skip separator line immediately following title
                i += 1
                continue
            
            elif stripped == '' and prev_was_heading:
                # After title, only keep one empty line
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        # New format: delete entire file folder
        if os.path.exists(folder_path) and os.path.isdir(folder_path):
            shutil.rmtree(folder_path)
            logger.info(f"Report folder deleted: {report_id}")
            return True
        
        # Backward compatible format: delete separate files
        deleted = False
