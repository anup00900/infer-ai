    ...
    <div class="main-split-layout">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  <div class="loading-icon">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <circle cx="12" cy="12" r="10" stroke-width="4" stroke="var(--border-primary)"></circle>
      ...
      <div class="right-panel" ref="rightPanel">
        ...
        <div class="workflow-overview" v-if="agentLogs.length > 0 || reportOutline">
          ...
          <div v-if="isComplete" class="download-actions">
...
.section-number {
  ...
  font-size: 16px;
  color: var(--text-muted); /* Dark gray, does not change with status */
  font-weight: 500;
}
...
.generated-content :deep(.md-h2) { font-size: 20px; border-bottom: 1px solid var(--bg-tertiary); padding-bottom: 8px; }
...
.right-panel {
  ...
  --wf-border: var(--border-primary);
  --wf-divider: var(--bg-tertiary);

  --wf-active-bg: var(--bg-secondary);
  --wf-active-border: var(--accent-blue);
  --wf-active-dot: var(--accent-blue);
  --wf-active-text: var(--text-primary);

  --wf-done-bg: var(--bg-secondary);
  --wf-done-border: var(--border-primary);
  --wf-done-dot: var(--success);

  --wf-muted-dot: var(--border-secondary);
...
.tool-badge.tool-purple {
  background: color-mix(in srgb, var(--accent-cyan) 14%, var(--bg-card));
...
.tool-badge.tool-green {
  ...
  border-color: color-mix(in srgb, var(--success) 40%, var(--border-primary));
  color: var(--success);
}
...
.tool-badge.tool-cyan {
  background: color-mix(in srgb, var(--accent-cyan) 12%, var(--bg-card));
...
:deep(.interview-display .agent-tab.active) {
  background: linear-gradient(135deg, color-mix(in srgb, var(--accent-blue) 14%, var(--bg-card)) 0%, color-mix(in srgb, var(--accent-blue) 6%, var(--bg-secondary)) 100%);
  border-color: var(--border-accent);
  color: var(--accent-blue);
...
:deep(.insight-header) {
  padding: 12px 16px;
  background: linear-gradient(135deg, color-mix(in srgb, var(--accent-blue) 12%, var(--bg-card)) 0%, color-mix(in srgb, var(--accent-cyan) 8%, var(--bg-secondary)) 100%);
...
:deep(.quicksearch-header) {
  padding: 12px 16px;
  background: linear-gradient(135deg, var(--accent-glow) 0%, color-mix(in srgb, var(--accent-blue) 10%, var(--bg-secondary)) 100%);
  border-radius: 8px 8px 0 0;
  border: 1px solid var(--border-accent);
  border-bottom: none;
}
...
.log-content::-webkit-scrollbar-thumb { background: var(--border-secondary); border-radius: 2px; }
...
.log-msg.warning { color: var(--accent-cyan); }
