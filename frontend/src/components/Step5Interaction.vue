        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">Quick chat version of Report Agent with 4 professional tools, has Infer's full memory</div>
              </div>
          ...
          <div class="chat-input-area">
            <textarea 
              ...
              :disabled="isSending || (!selectedAgent && chatTarget === 'agent') || (chatTarget === 'report_agent' && !props.simulationId)"
              rows="1"
              ref="chatInputRef"
            ></textarea>
            <button 
              class="send-btn"
              @click="sendMessage"
              :disabled="!chatInput.trim() || isSending || (!selectedAgent && chatTarget === 'agent') || (chatTarget === 'report_agent' && !props.simulationId)"
            >
...
const sendToReportAgent = async (message) => {
  if (!props.simulationId) {
    throw new Error('Simulation not loaded yet. Please wait for the report to load.')
  }
  ...
  if (res.success && res.data) {
    const rawResponse = res.data.response || res.data.answer
    const content = typeof rawResponse === 'object'
